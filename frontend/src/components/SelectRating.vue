  ...
  <div class="text-center">
    <v-menu>
      <template v-slot:activator="{ props }">
        ...
        <h2> {{ rating[itemSelected]?.title ?? 'undefined' }} </h2>
        </span>
      </template>

      <v-list>
        <v-list-item v-for="(item, index) in items" :key="index" @click="selectItem(item)" density="compact" rounded="lg"
          :class="item.value === itemSelected ? 'bg-blue-darken-4': ''">
